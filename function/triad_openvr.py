...
def get_np_matrix(pose_mat):
    m1 = np.array(pose_mat[0])
    m2 = np.array(pose_mat[1])
    m3 = np.array(pose_mat[2])

    T_matrix = np.vstack((m1, m2, m3))

    return T_matrix

def correct_rotation_matrix(mat):
    # Perform SVD
    U, _, Vt = np.linalg.svd(mat)
    # Correct the rotation matrix
    R = np.dot(U, Vt)
    # Ensure it is a proper rotation matrix
...
def convert_to_quaternion(pose_mat):
    ...
    # Ensure a consistent representation by keeping the scalar part (r_w) positive
    # if r_w < 0:
    #     r_w = -r_w
    #     r_x = -r_x
    #     r_y = -r_y
    ...
    ## 3rd implementation
    #
    # mat_to_convert = np.array([
    #     [pose_mat[0][0], pose_mat[0][1], pose_mat[0][2]],
    #     [pose_mat[1][0], pose_mat[1][1], pose_mat[1][2]],
    #     [pose_mat[2][0], pose_mat[2][1], pose_mat[2][2]]
    # ])
    #
    # mat_to_convert = correct_rotation_matrix(mat_to_convert)
    #
    # q8d = Quaternion(matrix=mat_to_convert)
    #
    # if q8d.w < 0:
    #     q8d = Quaternion(-q8d.w, -q8d.x, -q8d.y, -q8d.z)
    #
    # r_w = q8d.w
    # r_x = q8d.x
    # r_y = q8d.y
...
class vr_tracking_reference(vr_tracked_device):
    ...
    def get_absolute_pose_matrix(self, pose=None):
        if pose is None:
            pose = get_pose(self.vr)
        if pose[self.index].bPoseIsValid:
            return pose[self.index].mDeviceToAbsoluteTracking
...
class triad_openvr():
    def __init__(self, configfile_path='config.json'): 
